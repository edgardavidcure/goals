...
export async function updateGoal(
  ...
  try {
    const updatedGoal = await GoalModel.findByIdAndUpdate(goalId, data, {
      new: true,
    });
    return updatedGoal;
  } catch (error) {
...
export async function fetchCardData(userId: string) {
  ...
  try {
    const goalsCompletedPromise = getTotalCountGoalsByStatus(
      userId,
      "Completed",
    );
    const goalsPendingPromise = getTotalCountGoalsByStatus(userId, "Pending");
    const goalsInProgressPromise = getTotalCountGoalsByStatus(
      userId,
      "InProgress",
    );
...
async function getTotalCountGoalsByStatus(
  userId: string,
  status: string,
): Promise<number> {
  try {
